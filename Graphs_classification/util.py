...
class S2VGraph(object):
    def __init__(self, g, label, node_tags=None, node_features=None, device = device):
        '''
            g: a networkx graph
...
def load_data(dataset, degree_as_tag):
    ...
    for g in g_list:
        g.node_features = torch.zeros([len(g.node_tags), len(tagset)]).to(device)
        g.node_features[range(len(g.node_tags)), [tag2index[tag] for tag in g.node_tags]] = 1
        # print(np.unique(g.node_tags))
